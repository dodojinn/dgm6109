...
/*
  ...
  - D3.js library
     https://d3js.org/   
  - Code structure adapted from course materials.

  Description:
  This visualization explores the relationship between daily average temperature
  and total calorie intake over a period of time. Each bubble represents a day,
  with its position, size, and color encoding multiple dimensions of data:
  - X-axis:Average Temp (°C)
  - Y-axis:Total Calorie Intake (kcal)
  - Circle Size:Meals per Day
  - Color Intensity:Emotional Impact (1–5)
 */
...
/* Main drawing area, shifted by margins */
...
let data;
let xScale; // Average Temperature -> X Position
let yScale; // Total Calories -> Y Position
let rScale; // Meals per Day -> Circle Radius
let colorScale; // Emotional Impact -> Color
let xAxisGroup;
let yAxisGroup;
...
/*
  ...
  Requirements: Requires D3.js library and an SVG element in the HTML.
  Parameters:
  data (Array): The raw array of objects loaded from the JSON file.
  Returns:
  data (Array): Returns the raw data for potential debugging purposes.
 */
function buildVisualization(data) {
...
/*
 ...
 Parameters:
   data (Array): The raw array of objects loaded from the JSON file.
 Returns:
   cleaned (Array): The cleaned and organized array of objects ready for visualization.  
 */
function organizeData(data) {
  ...
  /* Filter out data beyond November 15, 2025 */
  cleaned = cleaned.filter(function (d) {
    return d.date <= "2025-11-15";
  });
  ...
  console.log("cleaned and filtered data:", cleaned);
  return cleaned;
}
...
/*
  ...
  Requirements: 'plotGroup' SVG group must be defined.
  Parameters:
    data (Array): The cleaned and organized array of objects ready for visualization.
  Returns: None.  
 */
...
function buildScales(data) {
  ...
  rScale = d3.scaleLinear()
    .domain([minMeals, maxMeals])
    .range([10, 20]);

  /* Emotional Impact (1–5 -> color from light to dark) */
...
/*
  ...
  Requirements: 'plotGroup', scales, and axes must be defined.
  Parameters:
    data (Array): The cleaned and organized array of objects ready for visualization.
  Returns: None.  
 */
function drawVisualization(data) {
  ...
  bubbles
    ...
    .append("circle")
    .attr("cx", function (d) {
      return xScale(d.avgTemp);
    })
    .attr("cy", function (d) {
      return yScale(d.totalCalories);
    })
    .attr("r", function (d) {
      return rScale(d.mealsPerDay);
    })
